  ...
  return (
    <div className={cn('w-full group relative', className)} ref={containerRef}>
      ...
      >
        ...
        <svg className="w-5 h-5 text-ink-faded opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="absolute z-50 mt-4 left-0 bg-[#e3d5b8] shadow-scroll p-4 min-w-[280px] rounded-sm animate-in fade-in zoom-in-95 duration-200 border border-[#2c1a0f]/10">
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center justify-between mb-4 border-b border-ink/20 pb-2">
                <button type="button" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-1 hover:text-wax transition-colors">
                  ←
                </button>
                <span className="font-heading font-bold text-ink text-sm tracking-wide">
                  {format(currentMonth, 'MMMM yyyy')}
                </span>
                <button type="button" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-1 hover:text-wax transition-colors">
                  →
                </button>
              </div>
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {weekDays.map((day) => (
                  <div key={day} className="text-[10px] font-serif text-ink-faded uppercase tracking-widest mb-2">
                    {day}
                  </div>
                ))}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected ? 'bg-wax text-[#f0e6d2] shadow-sm font-bold' : 'hover:bg-ink/10 text-ink',
