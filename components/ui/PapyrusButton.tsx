...
export interface PapyrusButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'ghost';
  size?: 'sm' | 'md' | 'lg';
}

export const PapyrusButton = React.forwardRef<HTMLButtonElement, PapyrusButtonProps>(
  ({ className, variant = 'primary', size = 'md', children, ...props }, ref) => {
    
    const baseStyles = 'relative inline-flex items-center justify-center font-heading font-bold uppercase tracking-widest transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed';
    ...
    const variants = {
      ...
      ghost: 'bg-transparent text-ink-faded hover:text-wax hover:underline decoration-wavy decoration-1 underline-offset-4',
    };
