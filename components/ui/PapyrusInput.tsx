...
export interface PapyrusInputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  error?: string;
}

export const PapyrusInput = React.forwardRef<HTMLInputElement, PapyrusInputProps>(
  ({ className, label, error, ...props }, ref) => {
    return (
      <div className="w-full group">
        ...
        />
        {error && (
          <p className="mt-1 text-sm text-red-800 font-serif italic pl-1">{error}</p>
        )}
      </div>
    );
  }
