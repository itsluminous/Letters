...
export const PapyrusScroll: React.FC<PapyrusScrollProps> = ({
  letter, mode, onSave, onEdit, onDelete, showActions = false, recipientOptions = [], contacts = [], className,
}) => {
  const [content, setContent] = useState(letter?.content || '');
  const [selectedRecipientId, setSelectedRecipientId] = useState(letter?.recipientId || '');
    ...
    <div className={cn(
      ...
      <div className="mb-8 border-b-2 border-ink/10 pb-4 flex justify-between items-end">
        <div className="font-heading text-ink">
            ...
            {!isCompose && letter?.recipient && (
               <p className="text-xs mt-1 text-ink-light">To: {letter.recipient.email}</p>
            )}
        </div>
      ...
      {(isCompose || isEditing) && (
        <div className="mt-8 flex justify-end gap-4 pt-6 border-t border-ink/5">
            {isEditing && !isCompose && (
                <button onClick={() => setIsEditing(false)} className="text-ink-light hover:text-ink font-heading uppercase text-xs font-bold tracking-widest">
                    Cancel
                </button>
            )}
            <PapyrusButton onClick={handleSave} disabled={isSaving || !content.trim()}>
                {isSaving ? 'Sealing...' : isCompose ? 'Seal & Send' : 'Save Corrections'}
            </PapyrusButton>
        </div>
      )}
      ...
      {mode === 'view' && showActions && (
         <div className="absolute top-8 right-8 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
             ...
             <button onClick={() => setIsEditing(true)} className="text-ink-light hover:text-wax">Edit</button>
         </div>
      )}
    </div>
  );
