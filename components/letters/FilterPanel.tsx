...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const handleContactChange = (value: string | string[]) =>
    onFilterChange({
      ...filters,
      contactIds: Array.isArray(value) ? value : [value],
    });
  const handleBeforeDateChange = (date: Date | null) =>
    onFilterChange({ ...filters, beforeDate: date });
  const handleAfterDateChange = (date: Date | null) =>
    onFilterChange({ ...filters, afterDate: date });
  const handleClearFilters = () =>
    onFilterChange({ contactIds: [], beforeDate: null, afterDate: null });
  ...
  return (
    <>
          ...
          <div className="bg-[#fdf6e3] p-4 border-b border-[#d4c4a8] animate-slide-up">
            ...
            {hasActiveFilters && (
              <button
                ...
                className="mt-4 text-sm text-wax font-bold hover:underline"
              >
                Clear Filters
              </button>
            )}
      ...
      <div
        className={cn(
          "hidden md:block bg-transparent border-b border-ink/10 pb-6 mb-6",
          className
        )}
      >
        <div className="flex items-center justify-between mb-4">
          ...
          >
            ...
            <span className="text-ink-light font-bold">
              {isOpen ? "−" : "+"}
            </span>
          </button>
        ...
        {isOpen && (
          <div className="animate-slide-up">
            ...
            {hasActiveFilters && (
              <button
                onClick={handleClearFilters}
                className="mt-4 text-xs text-wax font-bold hover:underline uppercase tracking-wide cursor-pointer"
              >
                Clear All
              </button>
            )}
