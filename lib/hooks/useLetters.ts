  ...
  const fetchLetters = useCallback(async () => {
    try {
      ...
      if (filters?.beforeDate) {
        unreadQuery = unreadQuery.lt('created_at', filters.beforeDate.toISOString());
      }
      if (filters?.afterDate) {
        unreadQuery = unreadQuery.gt('created_at', filters.afterDate.toISOString());
      }
      ...
      if (filters?.beforeDate) {
        readQuery = readQuery.lt('created_at', filters.beforeDate.toISOString());
      }
      if (filters?.afterDate) {
        readQuery = readQuery.gt('created_at', filters.afterDate.toISOString());
      }
